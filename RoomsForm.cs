...
{
    public partial class RoomsForm : MaterialForm
    {
        ...
        //Данные для формы
        private IEnumerable<Room> _data;
        ...
        private async void LoadData(string param)
        {
            try
            {
                ...
                {
                    _data = await _dataService.GetEntitiesAsync<Room>();
                    dgv.DataSource = _data;
                }
                else
                {
                    //иначе только то что удв. условиям
                    _data = await _dataService.GetEntitiesAsync<Room>(x => x.Title
                        .ToLower()
                        .Trim()
        ...
        private void RoomsFormLoad(object sender, EventArgs e)
        {
            LoadData(null);
        }
        ...
        {
            if (dgv.SelectedRows.Count <= 0) return;
            if (dgv.SelectedRows[0].DataBoundItem is Room item)
            {
        ...
        private async void DeleteBtnClick(object sender, EventArgs e)
        {
            ...
            if (dgv.SelectedRows[0].DataBoundItem is Room item) //получем обьект из выбранной строки
            {
