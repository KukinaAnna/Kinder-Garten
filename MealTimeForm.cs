...
{
    public partial class MealTimeForm : MaterialForm
    {
        ...
        //Данные для формы
        private IEnumerable<MealTime> _data;

        //Конструктор, который принимает абстракцию сервиса к доступу к данным
        public MealTimeForm(IDataService dataService)
        {
            ...
            _validator.AddValue(time_txbx, _validator.RequiredValidation, "Время обязательно");
            _validator.AddValue(time_txbx, _validator.IsTimeValidation, "Формат времени не верный");
            DisableControls();
        }
        ...
        private async void LoadData(string param)
        {
            try
            {
                ...
                {
                    _data = await _dataService.GetEntitiesAsync<MealTime>();
                    dgv.DataSource = _data;
                }
                else
                {
                    //иначе только то что удв. условиям
                    _data = await _dataService.GetEntitiesAsync<MealTime>(x => x.Title
                        .ToLower()
                        .Trim()
        ...
        private void MealTimesFormLoad(object sender, EventArgs e)
        {
            LoadData(null);
        }
        ...
        private async void SaveBtnClick(object sender, EventArgs e)
        {
            ...
            if (_validator.CheckAll())
            {
                if (_isEdit) // если вкл. редактирование
                {
                    ...
                    if (dgv.SelectedRows[0].DataBoundItem is MealTime item) // получаем выбранные элемент
                    {
                        item.Title = text_txbx.Text; // изменяем его содержимое
                        item.Time = TimeSpan.Parse(time_txbx.Text);
                        try
                        {
                ...
                {
                    var item = new MealTime() // создаем обьект и заполняем данными
                    {
                        Title = text_txbx.Text,
                        Time = TimeSpan.Parse(time_txbx.Text)
                };
                    try
                    {
        ...
        {
            if (dgv.SelectedRows.Count <= 0) return;
            if (dgv.SelectedRows[0].DataBoundItem is MealTime item)
            {
                text_txbx.Text = item.Title; // Заполняем контролы данными из обьекта
                time_txbx.Text = item.Time.ToString();
            }
            DisableControls();
        }
        ...
        private async void DeleteBtnClick(object sender, EventArgs e)
        {
            ...
            if (dgv.SelectedRows[0].DataBoundItem is MealTime item) //получем обьект из выбранной строки
            {
