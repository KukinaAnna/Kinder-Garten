...
{
    public partial class SpecializationsForm : MaterialForm
    {
        ...
        //Данные для формы
        private IEnumerable<Specialization> _data;

        //Конструктор, который принимает абстракцию сервиса к доступу к данным
        public SpecializationsForm(IDataService dataService)
        {
        ...
        private async void LoadData(string param)
        {
            try
            {
                ...
                {
                    _data = await _dataService.GetEntitiesAsync<Specialization>();
                    dgv.DataSource = _data;
                }
                else
                {
                    //иначе только то что удв. условиям
                    _data = await _dataService.GetEntitiesAsync<Specialization>(x => x.Title
                        .ToLower()
                        .Trim()
        ...
        private void SpecializationsFormLoad(object sender, EventArgs e)
        {
            LoadData(null);
        }
        ...
        private async void SaveBtnClick(object sender, EventArgs e)
        {
            ...
            if (_validator.CheckAll())
            {
                if (_isEdit) // если вкл. редактирование
                {
                    ...
                    if (dgv.SelectedRows[0].DataBoundItem is Specialization item) // получаем выбранные элемент
                    {
                ...
                {
                    var item = new Specialization() // создаем обьект и заполняем данными
                    {
        ...
        {
            if (dgv.SelectedRows.Count <= 0) return;
            if (dgv.SelectedRows[0].DataBoundItem is Specialization item)
            {
        ...
        private async void DeleteBtnClick(object sender, EventArgs e)
        {
            ...
            if (dgv.SelectedRows[0].DataBoundItem is Specialization item) //получем обьект из выбранной строки
            {
