...
{
    public partial class LessonsForm : MaterialForm
    {
        ...
        //Данные для формы
        private IEnumerable<Lesson> _data;

        //Конструктор, который принимает абстракцию сервиса к доступу к данным
        public LessonsForm(IDataService dataService)
        {
            ...
            descr_txbx.Font = MaterialSkinManager.Instance.ROBOTO_MEDIUM_10;
        }
        ...
        private async void LoadData(string param)
        {
            try
            {
                ...
                {
                    _data = await _dataService.GetEntitiesAsync<Lesson>();
                    dgv.DataSource = _data;
                }
                else
                {
                    //иначе только то что удв. условиям
                    _data = await _dataService.GetEntitiesAsync<Lesson>(x => x.Title
                        .ToLower()
                        .Trim()
        ...
        private void LessonsFormLoad(object sender, EventArgs e)
        {
            LoadData(null);
        }
        ...
        private async void SaveBtnClick(object sender, EventArgs e)
        {
            ...
            if (_validator.CheckAll())
            {
                if (_isEdit) // если вкл. редактирование
                {
                    ...
                    if (dgv.SelectedRows[0].DataBoundItem is Lesson item) // получаем выбранные элемент
                    {
                        item.Title = text_txbx.Text; // изменяем его содержимое
                        item.Description = descr_txbx.Text;
                        try
                        {
                ...
                {
                    var item = new Lesson() // создаем обьект и заполняем данными
                    {
                        Title = text_txbx.Text,
                        Description = descr_txbx.Text
                    };
                    try
                    {
        ...
        {
            if (dgv.SelectedRows.Count <= 0) return;
            if (dgv.SelectedRows[0].DataBoundItem is Lesson item)
            {
                text_txbx.Text = item.Title; // Заполняем контролы данными из обьекта
                descr_txbx.Text = item.Description;
            }
            DisableControls();
        }
        ...
        private async void DeleteBtnClick(object sender, EventArgs e)
        {
            ...
            if (dgv.SelectedRows[0].DataBoundItem is Lesson item) //получем обьект из выбранной строки
            {
