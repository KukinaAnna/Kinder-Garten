...
{
    public partial class MeasurementForm : MaterialForm
    {
        ...
        //Данные для формы
        private IEnumerable<Measurement> _data;

        //Конструктор, который принимает абстракцию сервиса к доступу к данным
        public MeasurementForm(IDataService dataService)
        {
        ...
        private async void LoadData(string param)
        {
            try
            {
                ...
                {
                    _data = await _dataService.GetEntitiesAsync<Measurement>();
                    dgv.DataSource = _data;
                }
                else
                {
                    //иначе только то что удв. условиям
                    _data = await _dataService.GetEntitiesAsync<Measurement>(x => x.Title
                        .ToLower()
                        .Trim()
        ...
        private void MeasurementsFormLoad(object sender, EventArgs e)
        {
            LoadData(null);
        }
        ...
        private async void SaveBtnClick(object sender, EventArgs e)
        {
            ...
            if (_validator.CheckAll())
            {
                if (_isEdit) // если вкл. редактирование
                {
                    ...
                    if (dgv.SelectedRows[0].DataBoundItem is Measurement item) // получаем выбранные элемент
                    {
                ...
                {
                    var item = new Measurement() // создаем обьект и заполняем данными
                    {
        ...
        {
            if (dgv.SelectedRows.Count <= 0) return;
            if (dgv.SelectedRows[0].DataBoundItem is Measurement item)
            {
        ...
        private async void DeleteBtnClick(object sender, EventArgs e)
        {
            ...
            if (dgv.SelectedRows[0].DataBoundItem is Measurement item) //получем обьект из выбранной строки
            {
